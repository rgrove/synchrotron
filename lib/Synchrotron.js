...
const normalizePathsToSync = require('./util/normalizePathsToSync');
const getParentPaths = require('./util/getParentPaths');
...
class Synchrotron extends EventEmitter {
  ...
  */
  ...
  _scheduleSync() {
    ...
    let delay = Math.min(
      Math.round(this.debounceMax),
      Math.round(this.debounceMin * Math.pow(2, this._debounceCount))
    );
...
/**
Emitted when an error occurs during a sync operation.

@event Synchrotron#error
@type {Error}
*/
