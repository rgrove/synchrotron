...
import normalizePathsToSync from './normalizePathsToSync.mjs';
...
class Synchrotron extends EventEmitter {
  ...
  @param {object} [options]
    ...
    @param {boolean} [options.deleteIgnored]
      When `true`, ignored files and directories will be deleted from the
      destination.
  ...
  */
  constructor(dest, source, options = {}) {
    ...
    this.deleteIgnored = deleteIgnored;
    this.dest = dest;
    this.dryRun = dryRun;
    this.log = logger;
  ...
  async sync(pathsToSync = [ '.' ]) {
    ...
    this._syncPromise = new Promise((resolve, reject) => {
      ...
      if (this.deleteIgnored) {
        rsyncArgs.push('--delete-excluded');
      }
      ...
      readline.createInterface({
        ...
        this.emit('rsyncStdout', { label, level, line, message });
      });
      ...
      readline.createInterface({
        ...
        this.emit('rsyncStderr', { level: Logger.LEVEL_WARN, line });
      });
      ...
      rsync.on('close', code => {
        ...
        if (RSYNC_RECOVERABLE_ERROR_CODES.has(code)) {
          ...
          this.emit('warning', {
            message: `rsync exited with code ${code}, but no worries! Synchrotron will recover automatically. :)`,
          });

          return void resolve(this.sync());
        }

        let err = new Error(`rsync exited with error code ${code}. Unfortunately this is a fatal error. :(`);
        this.emit('error', err);
        reject(err);
      });
...
@prop {string} level
  Log level appropriate for this message.
